-- Mealendar - Migration : rename planned_meals.covers_days -> planned_meals.covers_meals
--
-- Changement de semantique : avant 'covers_days' comptait en jours et couvrait
-- le meme slot dans les jours suivants (ex diner lundi avec covers_days=2
-- couvrait aussi diner mardi). Maintenant 'covers_meals' compte en NOMBRE DE
-- REPAS PRINCIPAUX (dejeuner + diner) couverts au total, en sautant les
-- petits-dejeuners et gouters.
--
-- Exemple nouveau : diner lundi avec covers_meals=2 -> couvre diner lundi
-- (le current) + dejeuner mardi (le prochain repas principal). Pas le diner
-- mardi.
--
-- La logique 'quels slots sont couverts' est calculee cote client/serveur a
-- partir du slot_config du foyer, pas en DB. La DB stocke juste l'entier.

alter table public.planned_meals
  rename column covers_days to covers_meals;

-- ============================================================================
-- RPC set_planning_meals : adaptee pour coversMeals
-- ============================================================================
...
-- ============================================================================
-- RPC update_planned_meal : supporte coversMeals dans le patch
-- ============================================================================
